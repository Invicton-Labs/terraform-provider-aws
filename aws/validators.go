...
func validateNoSignatureParameters(v interface{}, k string) (ws []string, errors []error) {
	for paramK := range v.(map[string]interface{}) {
		for _, filteredParam := range dataSourceSignedRequestFilteredParameters {
			if strings.ToLower(paramK) == strings.ToLower(filteredParam) {
				errors = append(errors, fmt.Errorf("%q cannot contain the key \"%s\"", k, paramK))
				break
			}
		}
	}
